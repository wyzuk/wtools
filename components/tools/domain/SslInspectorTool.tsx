    ...
    ? [
        ...
        { label: "Issuer Certificate Authority", value: [data.issuer?.O, data.issuer?.CN].filter(Boolean).join(" - ") || "Unknown" },
        {
          ...
          value: data.isExpired ? (
            ...
            <span className="font-mono text-[#0071e3] dark:text-blue-400 font-semibold text-xs">
        ...
        { label: "SHA-256 Fingerprint", value: data.fingerprint256, copyValue: data.fingerprint256, mono: true },
        {
          ...
          value: Array.isArray(data.subjectAltName) && data.subjectAltName.length > 0 ? (
          ...
          copyValue: Array.isArray(data.subjectAltName) ? data.subjectAltName.join(", ") : undefined,
        },
      ]
    : [];
