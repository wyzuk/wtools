...
export function WhoisTool() {
  ...
  const items = data
    ? [
        ...
        {
          label: "Domain Status",
          value: Array.isArray(data.status) && data.status.length > 0 ? (
        ...
        {
          label: "Nameservers",
          value: Array.isArray(data.nameservers) && data.nameservers.length > 0 ? (
            <div className="space-y-1 font-mono text-[#0071e3] dark:text-blue-400">
          ...
          copyValue: Array.isArray(data.nameservers) ? data.nameservers.join(", ") : undefined,
        },
      ]
    : [];
