...
// Pure JS MD5 implementation for client-side standard compatibility
...
export function HashGeneratorTool() {
  const [input, setInput] = useState("WTOOLS");
  ...
  useEffect(() => {
    ...
    computeHashes("WTOOLS");
  }, [computeHashes]);

  const handleGenerate = async () => {
    if (!input || limitReached) return;
    await computeHashes(input);
    setHasGenerated(true);
    await recordToolUsage();
  };

  const items = Object.entries(hashes).map(([algo, hashVal]) => ({
    label: algo,
    value: hashVal,
    copyValue: hashVal,
    mono: true,
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="p-6 bg-white dark:bg-[#0d0d0d] border border-slate-200 dark:border-white/10 rounded-xl space-y-4 shadow-sm dark:shadow-none transition-colors">
        <label htmlFor="hash-input" className="block text-xs font-semibold text-slate-700 dark:text-white/70 uppercase tracking-wider font-mono">
        ...
        <textarea
          ...
          placeholder="Type or paste any text to generate cryptographic hashes..."
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 pt-1">
          <div className="text-xs text-slate-500 dark:text-white/40 font-mono">
            <span>{input.length} characters &bull; {new TextEncoder().encode(input).length} bytes</span>
          </div>
          <button
            type="button"
      ...
      {items.length > 0 && (hasGenerated || !limitReached) && (
        <ResultCard
