...
export function UrlAnalyzerTool() {
  ...
  return (
    <div className="space-y-6">
      ...
      <form onSubmit={handleAnalyze} className="p-6 bg-white dark:bg-[#0d0d0d] border border-slate-200 dark:border-white/10 rounded-xl space-y-4 shadow-sm dark:shadow-none transition-colors">
        <div>
        ...
        <div className="flex flex-wrap items-center gap-2 text-xs text-slate-500 dark:text-white/50">
          ...
          {[
            ...
            <button
              ...
              className="px-2.5 py-1 rounded-md border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-white/5 text-slate-700 dark:text-white/70 hover:border-blue-500 hover:text-blue-600 dark:hover:text-white transition-colors truncate max-w-xs disabled:opacity-50"
            >
              {sample}
            </button>
          ))}
                  ...
                  <tbody className="divide-y divide-slate-100 dark:divide-white/5">
                    {parsed.params.map((p, idx) => (
                      <tr key={idx} className="hover:bg-slate-50 dark:hover:bg-white/5">
                        <td className="py-2.5 pr-4 text-blue-600 dark:text-blue-400 font-semibold">{p.key}</td>
